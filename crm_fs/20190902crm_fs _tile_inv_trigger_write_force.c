		...
		l2c_bug++;
		    ...
		    if(get_tileid() == 0 && tile_mask[get_tileid()])   /* TILE 0 */
			{
				...
				for(i=0; i<NUM_OF_CL; i++)
				{
					...
					{
						a = (*toGlobal(addr+k,1)); //read tile 1 data 80200000->B1200000 data : 803DEF20
					}
				}
				signal_remote(1,0,0x111ab000);

				// wait tile 1 rewrite invalide tile 0 L2C
				pull_signal_local(1,0x000ab111);
				...
				pull_signal_local(1,0x000ab111);
				// trigger force write through and get invalidate one line cache line
				for(i=0; i<NUM_OF_CL; i++)
				{
					for(k=0; k<WORDS_PER_CL; k++)
					{
						(*toGlobal(addr+k,1)) = 0xf00f0000 + i + k;  //write tile 1 data 80200000->B1200000 data : f00f0000
					}
				}
				signal_remote(1,0,0x11aab000); //2

				pull_signal_local(1,0x000ab111);

				for(i=0; i<NUM_OF_CL; i++)
				{
					for(k=0; k<WORDS_PER_CL; k++) /* dirty num = 8 */
					{
						a = (*toGlobal(addr+k,1)); //read tile 1 data 80200000->B1200000 data : 803DEF20
					}
				}
			...
			if (get_tileid() == 1 && tile_mask[get_tileid()])  /* TILE 1 */ //80700040 is tile 1 address
			{
				...
				// Signal to re-write trigger inv to tile 0 invalidate L2C
				// trigger INVs to tile 0

				for(i=0; i<NUM_OF_CL;i++)
					*(addr+i) = 0x02200000+i;
				signal_remote(0,1,0x000ab111);
				pull_signal_local(0,0x11aab000);  // wait until tile0 read and write data

				// invalidate in local L2C
				// tile 0 trying the write data to tile 1 to f00f0000 -f00f0007
				signal_remote(0,1,0x000ab111);
				// Signal to Trigger Individual WBs //write data from L2C to TLM
				pull_signal_local(0,0x11aab000); //2
				...
				for(i=0; i<NUM_OF_CL;i++)
					*(addr+1+i) = 0x11110001+i; // tile 1 location : 80200000 data: 11110000

				signal_remote(0,1,0x000ab111);
				pull_signal_local(0,0x11aab000);

				for(i=0; i<NUM_OF_CL; i++)
				{
					for(k=0; k<WORDS_PER_CL; k++) /* dirty num = 8 */
					{
						a = *(addr+k); //read tile 1 data 80200000->B1200000 data : 803DEF20
					}
				}
