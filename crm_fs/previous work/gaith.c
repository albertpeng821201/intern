...
void remote_checker(volatile unsigned int *addr, volatile unsigned int data)
{
...
{
	bootup_routine(0xf);
	//volatile unsigned char *addr_char = (volatile unsigned char *) addr2;
	//volatile unsigned short *addr_short = (volatile unsigned short *) addr2;
	volatile unsigned int *addr = addr2;
	...
	if(get_coreid() == 1)
	{
		...
		if(get_tileid() == 1 && tile_mask[get_tileid()])  /* TILE 1 */
		{
			...
			// Signal Trigger Individual WBs
			signal_remote(3,1,0x333ab111);
			pull_signal_local(3,0x111ab333);
			local_checker(addr4,3);
		}
		...
		if (*tile_id == 3 && tile_mask[get_tileid()])  /* TILE 3 */
		{
			volatile unsigned int i,j,k;

			pull_signal_local(1,0x333ab111);

			for(i=0,j=0; i<NUM_OF_CL; i++,j+=WORDS_PER_CL)
			{
				for(k=0; k<DIRTY_NUM; k++)
				{
					(*toGlobal(addr4+j+k,1)) =0xabcd0000 + i + k;
				}
			}

			signal_remote(1,3,0x111ab333);
