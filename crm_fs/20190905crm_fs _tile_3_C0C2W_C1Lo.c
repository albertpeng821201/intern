	...
	*/
	...
	if(get_tileid() == 0)
	{
		if(get_coreid() == 0)   /* core 0 */
		{
			...
			for(i=0; i<NUM_OF_CL; i++)
			{
				(*toGlobal(addr+i+0,1)) = 0xf00f000A + i;
				(*toGlobal(addr+i+1,1)) = 0xf00f000B + i;
				(*toGlobal(addr+i+2,1)) = 0xf00f000C + i;
				(*toGlobal(addr+i+3,1)) = 0xf00f000D + i;
			}
			signal_remote(1,0,0x000ab111);
		}
	}
	if(get_tileid() == 1)
	{
		if(get_coreid() == 0)   /* core 0 */
		{
			...
			//tile 0 tile 2 finish reading on tile TLM

			pull_signal_local(0,0x000ab111);
			pull_signal_local(2,0x222ab111);

			// tile 0 tile 2 start to re-write own L2C and write back to tile 1 TLM
			signal_remote(0,1,0x111ab000);
			signal_remote(2,1,0x111ab222);
			...
			// trigger tile 3 reading data from tile 1
	...
	if(get_tileid() == 3)
	{
		...
		if(get_coreid() == 0)   /* core 0 */
		{
			...
			signal_remote(1,2,0x222ab111);
			// re write new data in L2C
			pull_signal_local(1,0x111ab222);
			for(i=0; i<NUM_OF_CL; i++)
			{
				(*toGlobal(addr+i+4,1)) = 0xE00E000A + i;
				(*toGlobal(addr+i+5,1)) = 0xE00E000B + i;
				(*toGlobal(addr+i+6,1)) = 0xE00E000C + i;
				(*toGlobal(addr+i+7,1)) = 0xE00E000D + i;
			}
			signal_remote(1,2,0x222ab111);
		}
